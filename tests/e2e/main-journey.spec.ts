...
async function completeDiagnosticWithLowestScores(page: import('@playwright/test').Page) {
...
test('parcours accueil diagnostic résultats et PDF', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Votre niveau de préparation' })).toBeVisible()
  ...
  await expect(page.getByRole('button', { name: 'Imprimer le certificat' })).toBeVisible()
...
test('checklist et kit restent accessibles sans compte', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Télécharger ma checklist PDF' }).click()
  const checklistDownload = await checklistDownloadPromise

  expect(checklistDownload.suggestedFilename()).toBe('checklist-resilience-976.pdf')

  await openNavigationIfNeeded(page)
  await page.getByRole('link', { name: 'Kit' }).click()
  await expect(page.getByRole('heading', { name: 'Mon kit personnalisé' })).toBeVisible()
  await expect(
